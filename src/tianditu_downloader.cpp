...
bool TianDiTuDownloader::loadConfig(const std::string& configPath)
{
    ...
    try
    {
        ...
        _tileMode = jsonSolver.at("tileMode") == "ZYX" ? TileMode::ZYX : TileMode::ZXY;
        _extent = {jsonSolver.at("extent")[0],
                   jsonSolver.at("extent")[1],
                   jsonSolver.at("extent")[2],
                   jsonSolver.at("extent")[3]};
...
bool TianDiTuDownloader::downloadTile(TileIndex xy, int level, const std::string& key, const std::string& path)
{
    // 请求头
    ...
    if (res && res->status == 200)
    {
        ...
        out << res->body;
        out.close();
        LOG(INFO) << "[SUCCESS] tile " << level << "_" << xy.y << "_" << xy.x << " saved";
        return true;
    }
    else
    {
